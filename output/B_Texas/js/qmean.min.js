...
w){for(var d=parseInt(w.getAttribute("s")),h=0;h<a[f].scores.length;h++)if(d==a[f].scores[h][0]){var m=a[f].scores[h][1];if(m){w.setAttribute("q",a[f].scores[h][1]);c&&(.5>m?w.style.backgroundColor="rgba(255,80,0,.6)":(m=(m-.5)/.4,r=Math.round(255*(1-m)),b=Math.round(255*m),w.style.backgroundColor="rgba("+r+",90,"+b+",.6)"));break}}})}});g="";x=!1;if(0==k||k-1==f){g="";for(e=0;e<a[f].scores.length-1;e++)if(l=a[f].scores[e][0],(u=a[f].scores[e][1])&&a[f].scores[e+1][1]){if(""==g||x)g+="M"+(c+z*(l-
v))+" "+(w*(1-u)+h),x=!1;g+="L"+(c+(z*(l-v)+z))+" "+(w*(1-a[f].scores[e+1][1])+h)}else x=!0;g=t.path(g);g.attr({stroke:d[m],"stroke-width":n?1.5:2.5})}m=++m%8}n&&(n=!1,k--)}}}
